...
void loop() {
  ...
  // Serial.println("Vänster: "+String(currL)+"        Höger: "+String(currR)+"        a_max:"+String(a_max));
  ...
  if (Lwhisker & Rwhisker) {       //När båda nuddar, stanna, backa rakt tills endast en nuddar.
  ...
  else if (Lwhisker || recent_wall_left){     //Animation if the left whisker is touching or has recently been touched.
    ...
    else if ((currL >= 0 & currR <= 0) & ((t - recent_time)/1000000) <= 2.0) {
      a_max = 0.1;
      acc(currL, 0.03, dt, 1);
      acc(currR, -0.03, dt, 2);
    }
  ...
  else if (Rwhisker || recent_wall_right){ //Animation if the right whisker is touching or has recently been touched.
